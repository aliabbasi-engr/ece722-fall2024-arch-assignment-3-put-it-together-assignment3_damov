...
private:
  ...
  int    m_nbanks; // total number of banks for bankized caches or generally total number of caches at this level
  int    m_bankNumber; //this bank number of this cache
...
public:
  ...
  void SetBankNum (int bankNumber) {
  ...
  void LoadFromXml(TiXmlHandle root) {
     ...
     CacheRootPtr->QueryIntAttribute   ("nbanks"           , &m_nbanks          );
     CacheRootPtr->QueryIntAttribute   ("bankNumber"       , &m_bankNumber      );
